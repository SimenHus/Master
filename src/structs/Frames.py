...
class Frame(Common):
    ...
    def __init__(self, image: 'cv2.Mat', timestep: int, extractor: 'DataAssociation.Extractor', camera: 'Camera') -> None:
        ...
        self.map_points: dict[int: 'MapPoint'] = {} # frame map point id: map point
        self.outlier: dict[int: bool] = {} # Dict of outliers KeyPoint ID: is_outlier
...
class KeyFrame(Common):
    ...
    def get_map_points(self) -> set['MapPoint']:
        return {map_point for map_point in self.map_points.values()}

    def compute_scene_median_depth(self) -> float:
        if len(self.keypoints) == 0: return -1.0

        map_points = self.map_points.values()
        Tcw = self.get_pose()
    ...
    def get_map_point_matches(self) -> dict[int: 'MapPoint']: return self.map_points