...
class SymbolicJacobian:
    ...
    T_cam_landmark = T_cam.inverse() * landmark
    projection = K * T_cam_landmark
    pred = projection[:2] / projection[2]

    K2 = np.array([2407.240, 0, 1245.608, 0, 2404.539, 1004.407, 0, 0, 1])
    ...
    def subs(clc, T_rel: Pose3, T_ref: Pose3, landmark: Point3, jac):
        ...
        subs.update({k: v for k, v in zip(clc.K.to_storage(), clc.K2)})

        return jac.subs(subs).to_numpy()


class CameraExtrinsicPixelFactor(CustomFactor):
    def __init__(self, camera_key: int, ref_key: int, landmark_key: int, camera_intrinsics: np.ndarray, measurement: np.ndarray, noise_model: noiseModel = None) -> None:
