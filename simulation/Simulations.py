...
class SimulationBaseClass:
    ...
    def simulate_step(self) -> None:
        self.ISAM_step(self.current_step)

    def ISAM_step(self, i: int) -> None:
        raise Exception('Simulation function must be overwritten by child')
...
class Planar3D(SimulationBaseClass):
    ...
    def __init__(self, steps: int = 10) -> None:
        ...
        self.current_estimate.insert(self.key_start, SimulatedMeasurement.sample(self.trajectory.prior, Generator3D, Odometry3D)) # Insert noisy prior into estimates
    ...
    def ISAM_step(self, i: int) -> None:
        ...
        odometry_measurement = SimulatedMeasurement.sample(true_odom, Generator3D, Odometry3D) # Sample noisy odometry measurement

        if i % self.measurement_sample_rate == 0: # Check if external measurement is available
            measurement = SimulatedMeasurement.sample(true_state, Generator3D, Position3D).translation() # Sample noisy external measurement
...
#         self.odometry_noise = NoisyOdom2D.default_noise_model() # Import odometry noise model
#         self.measurement_noise = NoisyMeasurement2D.default_noise_model() # Import measurement noise model
#         self.prior_noise = self.odometry_noise # Define noise on prior estimate

#         self.measurement_sample_rate = 1

#         self.graph.push_back(PriorFactorPose2(self.key_start, self.trajectory.prior, self.prior_noise)) # Insert prior into graph
#         self.current_estimate.insert(self.key_start, NoisyOdom2D.sample(self.trajectory.prior)) # Insert noisy prior into estimates
...
#         odometry_measurement = NoisyOdom2D.sample(true_odom) # Sample noisy odometry measurement

#         if i % self.measurement_sample_rate == 0: # Check if external measurement is available
#             measurement = NoisyMeasurement3D.sample(true_state) # Sample noisy external measurement
#             self.graph.push_back(PositionFactor3D(key + 1, measurement, self.measurement_noise)) # Add measurement as factor to graph
